...
const useAddTest = (id) => {
            ...
            if (emptyFields?.length > 0) {
...

                {
                    Object.keys(formik?.values).map((key) => {
                        if (key === "start_at") {
                            formData?.append(key, new Date(formik?.values[key]).toISOString())
                        } else if (key === "session") {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: (info) => httpService.post(`app_admin/exams/upload/`, info, {
            headers: { 'Content-Type': question.type }
        }),
        onError: (error) => {
            console.log(error?.response?.data?.detail);
            toast?.error(error?.response?.data?.detail ? error?.response?.data?.detail : "Error occured Creating test")
        },
        onSuccess: async () => {
    ...
    const { mutate: singleQuestionMutate, isPending: loadingSingleQuestion, isSuccess: successSingleQuestion } = useMutation({
        mutationFn: (info) => httpService.post(`app_admin/exams/upload/${id}/single-question/`, info, {
    ...
    const { mutate: BulkMutate, isPending: loadingBulk, isSuccess: bulksuccess } = useMutation({
        mutationFn: (info) => httpService.post(`/app_admin/exams/upload/${id}/bulk-questions/`, info, {
            headers: { 'Content-Type': question.type }
        }),
        onError: (error) => {
            console.log(error?.response?.data?.detail);
